...
# 1. Update double-quoted string version (around line 2515)
search_double = (
...
content = content.replace(search_double, replace_double)

# 2. Update backtick string version (lines 4294, 4664)
# We will use regex to find the CRITICAL SYSTEM NOTE that starts with ` + "`exebox`" + `
pattern_backtick = r"CRITICAL SYSTEM NOTE: You are running inside an isolated background sandbox \(` \+ \"`exebox`\" \+ ` desktop\).*?(?=\n\n|\n`|\n\r?\n|$)"
...
content = re.sub(pattern_backtick, replace_backtick, content, flags=re.DOTALL)

# Re-apply B3, B4, B5 since we reverted main.go
# B3: pushTokenUsage wss->https, ws->http
push_token_search = r"backendURL := strings\.TrimSuffix\(connData\.BackendURL, \"/\"\)\s+targetURL := backendURL \+ \"/webhook/token-usage\""
push_token_replace = (
