...
idx_start = text.find('func executeGraphifyDAG')
idx_end = text.find('return finalResult, nil\n}') + len('return finalResult, nil\n}')

new_func = """func executeGraphifyDAG(ctx context.Context, command string) (string, error) {
...
new_text = text[:idx_start] + new_func + text[idx_end:]

with open('local-agent/dag_executor.go', 'w', encoding='utf-8') as f:
    f.write(new_text)
